...
import { SimpleSpanProcessor, BatchSpanProcessor, ConsoleSpanExporter, ParentBasedSampler, TraceIdRatioBasedSampler, } from '@opentelemetry/sdk-trace-base';
...
const telemetryInitation = (applicationName: string, port: number) => {
    ...
    const metricExporter = new PrometheusExporter({ port: port }, () => {
        console.log(`Find metrics at: http://localhost:${metricPort}${PrometheusExporter.DEFAULT_OPTIONS.endpoint}`);
    });
